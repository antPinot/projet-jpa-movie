...
public class IntegrationApp {
	...
	public static void main(String[] args) throws StreamReadException, DatabindException, IOException {
		...
		for (Film films : imdb.getFilms()) {
			...
			/*
			 * TypedQuery<Pays> queryLieuPays =
			 * em.createQuery("SELECT p FROM Pays p WHERE l.pays.nom =: param3",
			 * Pays.class); queryLieuPays.setParameter("param3",
			 * films.getLieuTournage().getPays().getNom()); List<Pays> lieuPaysResult =
			 * queryLieuPays.getResultList();
			 * 
			 * if (lieuPaysResult.size() == 0) {
			 * em.persist(films.getLieuTournage().getPays()); } else {
			...
			/*
			 * TypedQuery<Lieu> queryLieu = em.
			 * createQuery("SELECT l FROM Lieu l WHERE l.libelle= :param1 AND l.complement= :param2 AND l.pays.nom = :param3"
			 * , Lieu.class); queryLieu.setParameter("param1",
			 * films.getLieuTournage().getLibelle()); queryLieu.setParameter("param2",
			 * films.getLieuTournage().getComplement()); queryLieu.setParameter("param3",
			 * films.getLieuTournage().getPays().getNom()); List<Lieu> lieuResult =
			 * queryLieu.getResultList();
			 * 
			 * if (lieuResult.size() == 0) {
			 * 
			 * em.persist(films.getLieuTournage()); } else {
			...
			for (Role roles : films.getRoles()) {
				...
				/*
				 * TypedQuery<Role> queryRole =
				 * em.createQuery("SELECT r FROM Role r WHERE r.characterName = :param4",
				 * Role.class); queryRole.setParameter("param4", roles.getCharacterName());
				 * List<Role> rolesResult = queryRole.getResultList();
				 * 
				 * if (rolesResult.size() == 0) { em.persist(roles); } else {
			...
			for (Genre genres : films.getGenres()) {
				/*
				 * TypedQuery<Genre> queryGenre =
				 * em.createQuery("SELECT g FROM Genre g WHERE g.libelle = :param5",
				 * Genre.class); queryGenre.setParameter("param5", genres.getLibelle());
				 * List<Genre> genresResult = queryGenre.getResultList();
				 * 
				 * if (genresResult.size() == 0) { em.persist(genres); } else {
			...
			for (Realisateur realisateurs : films.getRealisateurs()) {
				 ...
				 * "SELECT real FROM Realisateur real WHERE real.identite = :param6",
				 * Realisateur.class); queryRealisateur.setParameter("param6",
				 * realisateurs.getIdentite()); List<Realisateur> realisateursResult =
				 * queryRealisateur.getResultList();
				 * 
				 * if (realisateursResult.size() == 0) { em.persist(realisateurs); } else {
			...
			for (Acteur acteurs : films.getActeurs()) {
				/*
				 * TypedQuery<Acteur> queryActeur =
				 * em.createQuery("SELECT a FROM Acteur a WHERE a.identite = :param7",
				 * Acteur.class); queryActeur.setParameter("param7", acteurs.getIdentite());
				 ...
				 * .createQuery("SELECT a FROM Acteur a WHERE a.lieuNaissance = :param8",
				 * Lieu.class); queryActeurLieu.setParameter("param8",
				 * acteurs.getLieuNaissance()); List<Lieu> acteursLieuResult =
				 * queryActeurLieu.getResultList();
				 */
				/*
				 * TypedQuery<Pays> queryActeurPays = em
				 * .createQuery("SELECT p FROM Pays p WHERE a.lieuNaissance.pays = :param9",
				 * Pays.class); queryActeur.setParameter("param9",
				 * acteurs.getLieuNaissance().getPays()); List<Pays> acteursPaysResult =
				 * queryActeurPays.getResultList();
				 */

				/*
				 * if (acteursResult.size() == 0 && acteursLieuResult.size() == 0 &&
				 * acteursPaysResult.size() == 0) { em.persist(acteurs);
				 * em.persist(acteurs.getLieuNaissance());
				 * em.persist(acteurs.getLieuNaissance().getPays()); } else if
				 * (acteursResult.size() == 1 && acteursLieuResult.size() == 1 &&
			...
			/*
			 * for (Acteur acteurs : films.getActeurs()) { em.persist(acteurs);
			 * em.persist(acteurs.getLieuNaissance());
			 * em.persist(acteurs.getLieuNaissance().getPays()); }
			 */
